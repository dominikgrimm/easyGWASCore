...
int main(int argc, char* argv[]) {
	...
	if(argc<5) {
		...
		logging(WARNING,"");
		logging(WARNING,"emmax <plink genotype file> <plink phenotype file> <float minor allele frequency filter> <output_folder> [permutations - optional]");
		logging(WARNING,"");
		exit(-1);
	}
	...
	*/
	...
	for(uint i=0; i<data.phenotype_names.size(); i++) {
		...
		begin = clock();
		logger.log(STATUS,"Running EMMAX for phenotype: " + data.phenotype_names[i]);
		CSingleTraitGWAS::EMMAX gwas(tmpData.Y.col(i),tmpData.X,tmpData.K);
		gwas.setBrent(true);
		if(permutations==0) {
